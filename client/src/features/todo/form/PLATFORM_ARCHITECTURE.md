...
- iOS `InputAccessoryView`는 “포커스된 TextInput”의 accessory로만 표시됩니다.
- 그래서 iOS는 `QuickModeContent.ios`를 **상시 마운트**해 두고, Quick Mode가 열릴 때만 host `TextInput`에 focus를 줘서 “첫 번째 탭에서 키보드만 뜨는 현상”을 방지합니다.
- Android/Web은 `mode === 'QUICK'`일 때만 Quick UI를 마운트합니다.
...
<br>(키보드 높이 추적 + `translateY`) | **Bottom Sheet**<br>
...
* **iOS 구현:** RN 내장 **`InputAccessoryView`** + 화면 밖(host) `TextInput`로 bar를 키보드에 부착.
  * `QuickModeContent.ios`: host `TextInput` + `InputAccessoryView`
  * `QuickContainer.ios`: dim backdrop만 담당 (배경 탭 시 닫기)
* **Android 구현:** `react-native-keyboard-controller` 기반 키보드 높이 추적 + Reanimated `translateY`로 Bar 이동.
  * `QuickContainer`: backdrop + `useKeyboardHandler`로 `keyboardHeight`를 구하고, `transform: translateY(-height)` 적용
* **Web 구현:** `position: fixed` 오버레이 + backdrop(dim) 클릭 시 닫기.
...
3. **State Change:** 짧은 딜레이 후 `store.openDetail(null, target)` 실행.
4. **iOS Close Suppress:** iOS는 Quick→Detail 전환 중 `keyboardDidHide`가 발생해도 Quick Mode가 닫히지 않도록 1회 suppress.
5. **Transition:** Quick Mode → Detail Mode 전환.
...
│   └── todo/
│       ├── form/
│       │   ├── GlobalFormOverlay.js          # [진입점] 전역 오버레이 (플랫폼 분기)
│       │   ├── containers/
...
│       │   │   ├── QuickInput.js             # Quick input (Android/Web)
│       │   │   └── QuickInput.ios.js         # Quick input UI (iOS)
│       │   └── content/
│       │       └── DetailContent.js          # 상세 폼 컨텐츠
...
* [ ] **Android Keyboard (Expo):** `client/app.json`에 `android.softwareKeyboardLayoutMode: "resize"`가 설정되었는가?
* [ ] **Android Detail Sheet:** `DetailContainer.android`에서 `android_keyboardInputMode="adjustResize"`가 설정되었는가?
* [ ] **Keyboard Sync:** iOS(`InputAccessoryView`) / Android(키보드 높이 추적)에서 Quick Bar가 키보드와 함께 자연스럽게 움직이는가?
* [ ] **Backdrop Dismiss:** dim(배경) 클릭/탭 시 Quick Mode가 닫히는가?
* [ ] **Transition:** Quick -> Detail 전환 시 flicker 없이 자연스럽게 전환되는가?
* [ ] **Hybrid Mount:** iOS `QuickModeContent.ios`는 상시 마운트 + `visible`일 때만 focus, 나머지 플랫폼은 `mode`에 따라 mount/unmount 되는가?
