...
export function formatHeaderYearMonth(year, month1Based, language) {
  const lang = resolveCalendarLanguage(language);

  if (lang === 'en') {
    return `${MONTH_SHORT_EN[month1Based - 1]} ${year}`;
  }

  if (lang === 'ja') {
    return `${year}年 ${month1Based}月`;
  }

  return `${year}년 ${month1Based}월`;
}

export function formatShortMonthLabel(month1Based, language) {
  const lang = resolveCalendarLanguage(language);

  if (lang === 'en') {
    return `${MONTH_SHORT_EN[month1Based - 1]}`;
  }
