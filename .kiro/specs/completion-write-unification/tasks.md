...
- [x] 2. Sync: stage 결과를 entity-aware invalidation 가능 형태로 확장
...
- [x] 3. Sync Coordinator: rerun latch 추가
  - `client/src/services/sync/index.js`
  - `needsResyncRef` 추가
  - sync 실행 중 들어온 새 `syncAll()` 요청을 run 종료 직후 1회 재실행

- [x] 4. Cache: completion-aware invalidation helper 추가
...
- [x] 5. Client: `useToggleCompletion`을 always-pending으로 전환
...
- [x] 6. Sync: post-sync invalidation 분기 적용
  - `client/src/services/sync/index.js`
...
- [ ] 7. Validation: recovery / race 시나리오 보강
  ...
  - [ ] 반복 on/off
  - [x] sync 실행 중 toggle
  - [x] app restart with pending backlog
  - [x] rapid toggle 최종 상태 검증
...
- [x] Checkpoint A: Sync coordinator safety 확보
  - Tasks 2~3 완료
  - sync 중 추가 pending이 후속 run으로 이어짐

- [x] Checkpoint B: Completion write 통일 완료
  - Tasks 4~6 완료
...
1. [x] (온라인) 비반복 todo completion on -> 즉시 반영 -> background sync -> 서버 수렴
2. [x] (온라인) 비반복 todo completion off -> 즉시 반영 -> background sync -> 서버 수렴
3. [x] (오프라인) 비반복 completion on/off -> 즉시 반영 -> 온라인 복귀 후 서버 수렴
4. [ ] (오프라인) 반복 todo completion on/off -> 날짜별 key 기준으로 서버 수렴
5. [x] (sync 중) completion toggle -> 현재 sync 종료 후 후속 sync가 자동 실행되는지 확인
...
8. [x] (app restart) pending completion backlog가 재시작 후 replay되는지 확인
9. [x] (rapid toggle) 최종 completed state가 로컬/서버에서 일치하는지 확인
