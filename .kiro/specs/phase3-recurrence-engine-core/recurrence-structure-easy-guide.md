...
1. 앱 시작
2. SQLite 초기화 [x] => 점포 준비
3. 동기화 서비스 시작 (백그라운드, 화면 렌더를 막지 않음) [x] => 본사-점포 물류를 왕복 자동 처리하고(점포->본사 전송 + 본사->점포 반영), 점포 창고(SQLite)를 최신 상태로 맞춤
4. 공통 조회/집계 레이어 [x] => 창고에서 물건을 넓게 꺼낸 뒤, 판매 가능한 것만 규칙으로 선별하고(반복 판정), 상태표(완료/카테고리)까지 붙여 공용 리스트로 정리
...
5. 화면 어댑터가 화면별 형태로 변환 [x] => 진열 전에 매대 타입별로 분류 (리스트형/캘린더형/스트립형)
6. todo-screen / todo-calendar / strip-calendar 렌더 [~] => 실제 매대에 보기 좋게 진열 (strip은 로딩 범위/캐시 타이밍 튜닝 진행 중)

해석 메모(중요):

1-1. 현재 후속 작업(예: 조회 범위/프리패치/캐시 정책 통합)은 "5번(화면어댑터 신규 기능 추가)"가 아니라 "6번(렌더 체감 품질/운영 안정화)"에 해당한다.

1-2. 
“매대를 새로 만드는 작업”이 아니라, “재고 보충 시스템을 표준화하는 작업”

지금 상태:
각 매대(TodoScreen/TodoCalendar/Strip)가 자기 방식대로 창고에서 물건 가져와서, 어떤 매대는 빨리 채워지고 어떤 매대는 늦게 채워짐
하려는 작업:
창고 조회/보충 규칙(범위, 프리패치, 캐시 무효화)을 공통으로 맞춰서, 매대마다 채워지는 속도/일관성을 맞춤
유지되는 것:
매대 진열 모양(화면 어댑터 결과)은 그대로 다름
문서용 한 줄:
“화면 기능 추가가 아니라, 공통 재고(캐시) 운영 정책을 통합해 진열(렌더) 품질을 안정화하는 작업”

2. 즉, 5번은 완료 상태를 유지하고, 6번에서 로딩 지연/범위 정책/캐시 일관성 개선을 진행한다.
3. 관련 검토 노트: `.kiro/specs/screen-adapter-layer/cache-policy-unification.md`
