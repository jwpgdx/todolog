...
기준일: 2026-09-21. 기존 명시 결정과 문서를 정리한 색인이다. 이번 감사로 새로운 UX를 freeze하지 않는다.
...
| ID | 질문 | 현재 근거 / 처리 방향 |
|---|---|---|
| D02 | 선택 중 todo 삭제·필터 변경·sync로 항목이 사라지면 선택 집합을 어떻게 처리할지 | 현재 ID 누락을 조용히 건너뛴다. all-or-nothing만으로 누락 정책까지 정해지지 않음. stale 항목 안내·재선택·실행 차단 조건 결정 |
| D03 | TodoScreen 선택모드에서 RN title/calendar를 어떻게 숨기거나 유지할지 | in-place 우선 후보이나 전환 spike 후 freeze하기로 보류. 문서 토론 뒤 기기 검증 필요 |
...
한 번에 모든 질문을 다시 묻지 않는다. D02처럼 다음 작업에 영향을 주는 미결정부터 확인하고, 사용자의 결정으로 정해진 항목은 반복 토론하지 않는다. D03/D04는 실제 사용 감각을 확인할 prototype이 필요할 수 있다.
