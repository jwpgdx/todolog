...
기준일: 2026-09-25. 기존 명시 결정과 문서를 정리한 색인이다. 2026-09-21 감사 이후 웹 GPT에서 사용자와 확정한 결정도 여기에 추가한다.
...
| F07 | iOS header 메뉴는 system pull-down/menu, Android app-bar `⋮`는 platform overflow menu, row `⋮`는 anchored native/Material menu를 기본으로 한다. Bottom sheet는 더 큰 task/selection surface가 필요한 경우에만 사용한다. 메뉴 데이터 계약은 공유한다. | triage header menu + 2026-09-25 D04 |
